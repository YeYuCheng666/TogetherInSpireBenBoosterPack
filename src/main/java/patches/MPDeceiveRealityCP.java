...
public class MPDeceiveRealityCP {
    public static boolean IsDeceiveReality(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof DeceiveReality){
                return true;
            }
            return false;
        }
    ...
    public static class DeceiveRealityCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDeceiveRealityCP.IsDeceiveReality(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDeceiveRealityCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = DeceiveReality.class,
            method = "use"
    )
    public static class DeceiveRealityUseCP {
        public static SpireReturn Prefix(DeceiveReality __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addBlock(__instance.block);
                for(int i = 0; i < 1; ++i) {
                    ((CharacterEntity) m).addCard(NetworkCard.Generate(new Safety()), CardGroup.CardGroupType.HAND);
                }
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDeceiveRealityCP.IsDeceiveReality(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
