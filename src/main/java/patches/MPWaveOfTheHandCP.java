...
public class MPWaveOfTheHandCP {
    public static boolean IsWaveOfTheHand(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof WaveOfTheHand){
                return true;
            }
            return false;
        }
    ...
    public static class WaveOfTheHandCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPWaveOfTheHandCP.IsWaveOfTheHand(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPWaveOfTheHandCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = WaveOfTheHand.class,
            method = "use"
    )
    public static class WaveOfTheHandUseCP {
        public static SpireReturn Prefix(WaveOfTheHand __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new WaveOfTheHandPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPWaveOfTheHandCP.IsWaveOfTheHand(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
