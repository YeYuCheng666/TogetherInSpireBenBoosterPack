...
public class MPDeflectCP {
    public static boolean IsDeflect(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Deflect){
                return true;
            }
            return false;
        }
    ...
    public static class DeflectCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDeflectCP.IsDeflect(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDeflectCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class DeflectUseCP {
        public static SpireReturn Prefix(Deflect __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDeflectCP.IsDeflect(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
