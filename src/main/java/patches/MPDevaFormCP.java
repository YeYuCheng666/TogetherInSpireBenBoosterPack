...
public class MPDevaFormCP {
    public static boolean IsDevaForm(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof DevaForm){
                return true;
            }
            return false;
        }
    ...
    public static class DevaFormCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDevaFormCP.IsDevaForm(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDevaFormCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class DevaFormUseCP {
        public static SpireReturn Prefix(DevaForm __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new DevaPower(m));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDevaFormCP.IsDevaForm(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
