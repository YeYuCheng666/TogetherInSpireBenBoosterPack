...
public class MPDefend_RedCP {
    public static boolean IsDefend_Red(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Defend_Red){
                return true;
            }
            return false;
        }
    ...
    public static class Defend_RedCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDefend_RedCP.IsDefend_Red(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDefend_RedCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class Defend_RedUseCP {
        public static SpireReturn Prefix(Defend_Red __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDefend_RedCP.IsDefend_Red(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
