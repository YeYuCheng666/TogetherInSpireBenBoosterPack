...
public class MPDoubleTapCP {
    public static boolean IsDoubleTap(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof DoubleTap){
                return true;
            }
            return false;
        }
    ...
    public static class DoubleTapCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDoubleTapCP.IsDoubleTap(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDoubleTapCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class DoubleTapUseCP {
        public static SpireReturn Prefix(DoubleTap __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new DoubleTapPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDoubleTapCP.IsDoubleTap(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
