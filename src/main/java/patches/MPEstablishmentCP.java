...
public class MPEstablishmentCP {
    public static boolean IsEstablishment(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class EstablishmentCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPEstablishmentCP.IsEstablishment(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPEstablishmentCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = Establishment.class,
            method = "use"
    )
    public static class EstablishmentUseCP {
        public static SpireReturn Prefix(Establishment __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new EstablishmentPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPEstablishmentCP.IsEstablishment(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
