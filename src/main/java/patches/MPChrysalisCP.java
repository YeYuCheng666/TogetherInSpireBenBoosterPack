...
public class MPChrysalisCP {
    public static boolean IsChrysalis(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class ChrysalisCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPChrysalisCP.IsChrysalis(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPChrysalisCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ChrysalisUseCP {
        public static SpireReturn Prefix(Chrysalis __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPChrysalisCP.IsChrysalis(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
