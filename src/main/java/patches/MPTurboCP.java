...
public class MPTurboCP {
    public static boolean IsTurbo(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class TurboCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPTurboCP.IsTurbo(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPTurboCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class TurboUseCP {
        public static SpireReturn Prefix(Turbo __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).gainEnergy(__instance.magicNumber);
                for(int i = 0; i < 1; ++i) {
                    ((CharacterEntity) m).addCard(NetworkCard.Generate(new VoidCard()), CardGroup.CardGroupType.DISCARD_PILE);
                }
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPTurboCP.IsTurbo(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
