...
public class MPWhiteNoiseCP {
    public static boolean IsWhiteNoise(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof WhiteNoise){
                return true;
            }
            return false;
        }
    ...
    public static class WhiteNoiseCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPWhiteNoiseCP.IsWhiteNoise(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPWhiteNoiseCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class WhiteNoiseUseCP {
        public static SpireReturn Prefix(WhiteNoise __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPWhiteNoiseCP.IsWhiteNoise(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
