...
public class MPPerseveranceCP {
    public static boolean IsPerseverance(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Perseverance){
                return true;
            }
            return false;
        }
    ...
    public static class PerseveranceCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPPerseveranceCP.IsPerseverance(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPPerseveranceCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class PerseveranceUseCP {
        public static SpireReturn Prefix(Perseverance __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPPerseveranceCP.IsPerseverance(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
