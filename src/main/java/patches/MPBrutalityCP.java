...
public class MPBrutalityCP {
    public static boolean IsBrutality(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Brutality){
                return true;
            }
            return false;
        }
    ...
    public static class BrutalityCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBrutalityCP.IsBrutality(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBrutalityCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class BrutalityUseCP {
        public static SpireReturn Prefix(Brutality __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new BrutalityPower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBrutalityCP.IsBrutality(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
