...
public class MPPanicButtonCP {
    public static boolean IsPanicButton(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof PanicButton){
                return true;
            }
            return false;
        }
    ...
    public static class PanicButtonCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPPanicButtonCP.IsPanicButton(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPPanicButtonCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = PanicButton.class,
            method = "use"
    )
    public static class PanicButtonUseCP {
        public static SpireReturn Prefix(PanicButton __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addBlock(__instance.block);
                m.addPower(new NoBlockPower(m, __instance.magicNumber, false));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPPanicButtonCP.IsPanicButton(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
