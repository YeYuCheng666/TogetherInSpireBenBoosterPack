...
public class MPCrescendoCP {
    public static boolean IsCrescendo(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Crescendo){
                return true;
            }
            return false;
        }
    ...
    public static class CrescendoCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPCrescendoCP.IsCrescendo(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPCrescendoCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class CrescendoUseCP {
        public static SpireReturn Prefix(Crescendo __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).switchStance(new WrathStance());
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPCrescendoCP.IsCrescendo(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
