...
public class MPConserveBatteryCP {
    public static boolean IsConserveBattery(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof ConserveBattery){
                return true;
            }
            return false;
        }
    ...
    public static class ConserveBatteryCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPConserveBatteryCP.IsConserveBattery(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPConserveBatteryCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ConserveBatteryUseCP {
        public static SpireReturn Prefix(ConserveBattery __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addBlock(__instance.block);
                m.addPower(new EnergizedBluePower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPConserveBatteryCP.IsConserveBattery(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
