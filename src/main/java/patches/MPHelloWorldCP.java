...
public class MPHelloWorldCP {
    public static boolean IsHelloWorld(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class HelloWorldCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPHelloWorldCP.IsHelloWorld(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPHelloWorldCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class HelloWorldUseCP {
        public static SpireReturn Prefix(HelloWorld __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new HelloPower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPHelloWorldCP.IsHelloWorld(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
