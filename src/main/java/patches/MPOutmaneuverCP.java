...
public class MPOutmaneuverCP {
    public static boolean IsOutmaneuver(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Outmaneuver){
                return true;
            }
            return false;
        }
    ...
    public static class OutmaneuverCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPOutmaneuverCP.IsOutmaneuver(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPOutmaneuverCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class OutmaneuverUseCP {
        public static SpireReturn Prefix(Outmaneuver __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                if (!__instance.upgraded) {
                    m.addPower(new EnergizedPower(m, 2));
                } else {
                    m.addPower(new EnergizedPower(m, 3));
                }
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPOutmaneuverCP.IsOutmaneuver(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
