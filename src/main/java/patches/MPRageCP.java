...
public class MPRageCP {
    public static boolean IsRage(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Rage){
                return true;
            }
            return false;
        }
    ...
    public static class RageCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPRageCP.IsRage(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPRageCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class RageUseCP {
        public static SpireReturn Prefix(Rage __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new RagePower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPRageCP.IsRage(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
