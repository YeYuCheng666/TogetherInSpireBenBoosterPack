...
public class MPPreparedCP {
    public static boolean IsPrepared(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class PreparedCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPPreparedCP.IsPrepared(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPPreparedCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class PreparedUseCP {
        public static SpireReturn Prefix(Prepared __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPPreparedCP.IsPrepared(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
