...
public class MPDemonFormCP {
    public static boolean IsDemonForm(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof DemonForm){
                return true;
            }
            return false;
        }
    ...
    public static class DemonFormCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDemonFormCP.IsDemonForm(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDemonFormCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class DemonFormUseCP {
        public static SpireReturn Prefix(DemonForm __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new DemonFormPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDemonFormCP.IsDemonForm(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
