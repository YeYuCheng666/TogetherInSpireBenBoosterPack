...
public class MPMetallicizeCP {
    public static boolean IsMetallicize(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Metallicize){
                return true;
            }
            return false;
        }
    ...
    public static class MetallicizeCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPMetallicizeCP.IsMetallicize(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPMetallicizeCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = Metallicize.class,
            method = "use"
    )
    public static class MetallicizeUseCP {
        public static SpireReturn Prefix(Metallicize __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new MetallicizePower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPMetallicizeCP.IsMetallicize(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
