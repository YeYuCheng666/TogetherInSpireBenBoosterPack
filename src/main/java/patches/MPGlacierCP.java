...
public class MPGlacierCP {
    public static boolean IsGlacier(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Glacier){
                return true;
            }
            return false;
        }
    ...
    public static class GlacierCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPGlacierCP.IsGlacier(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPGlacierCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class GlacierUseCP {
        public static SpireReturn Prefix(Glacier __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPGlacierCP.IsGlacier(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
