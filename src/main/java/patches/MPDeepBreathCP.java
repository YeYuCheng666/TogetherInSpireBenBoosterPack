...
public class MPDeepBreathCP {
    public static boolean IsDeepBreath(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof DeepBreath){
                return true;
            }
            return false;
        }
    ...
    public static class DeepBreathCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDeepBreathCP.IsDeepBreath(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDeepBreathCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class DeepBreathUseCP {
        @SpireInsertPatch(rloc = 4)
        public static SpireReturn Insert(DeepBreath __instance, AbstractPlayer p, AbstractMonster m)
        {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDeepBreathCP.IsDeepBreath(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
