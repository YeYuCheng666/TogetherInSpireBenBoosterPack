...
public class MPEvaluateCP {
    public static boolean IsEvaluate(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class EvaluateCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPEvaluateCP.IsEvaluate(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPEvaluateCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class EvaluateUseCP {
        public static SpireReturn Prefix(Evaluate __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPEvaluateCP.IsEvaluate(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
