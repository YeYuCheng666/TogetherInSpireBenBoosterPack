...
public class MPMagnetismCP {
    public static boolean IsMagnetism(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Magnetism){
                return true;
            }
            return false;
        }
    ...
    public static class MagnetismCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPMagnetismCP.IsMagnetism(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPMagnetismCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class MagnetismUseCP {
        public static SpireReturn Prefix(Magnetism __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new MagnetismPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPMagnetismCP.IsMagnetism(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
