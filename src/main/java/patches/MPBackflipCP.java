...
public class MPBackflipCP {
    public static boolean IsBackflip(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Backflip){
                return true;
            }
            return false;
        }
    ...
    public static class BackflipCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBackflipCP.IsBackflip(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBackflipCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class BackflipUseCP {
        public static SpireReturn Prefix(Backflip __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBackflipCP.IsBackflip(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
