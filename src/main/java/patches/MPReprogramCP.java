...
public class MPReprogramCP {
    public static boolean IsReprogram(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Reprogram){
                return true;
            }
            return false;
        }
    ...
    public static class ReprogramCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPReprogramCP.IsReprogram(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPReprogramCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ReprogramUseCP {
        public static SpireReturn Prefix(Reprogram __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new FocusPower(m, -__instance.magicNumber));
                m.addPower(new StrengthPower(m, __instance.magicNumber));
                m.addPower(new DexterityPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPReprogramCP.IsReprogram(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
