...
public class MPBurningPactCP {
    public static boolean IsBurningPact(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof BurningPact){
                return true;
            }
            return false;
        }
    ...
    public static class BurningPactCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBurningPactCP.IsBurningPact(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBurningPactCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = BurningPact.class,
            method = "use"
    )
    public static class BurningPactUseCP {
        public static SpireReturn Prefix(BurningPact __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).exhaust(1, false, false, false);
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBurningPactCP.IsBurningPact(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
