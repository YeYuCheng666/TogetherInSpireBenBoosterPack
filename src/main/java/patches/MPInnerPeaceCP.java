...
public class MPInnerPeaceCP {
    public static boolean IsInnerPeace(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof InnerPeace){
                return true;
            }
            return false;
        }
    ...
    public static class InnerPeaceCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPInnerPeaceCP.IsInnerPeace(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPInnerPeaceCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = InnerPeace.class,
            method = "use"
    )
    public static class InnerPeaceUseCP {
        public static SpireReturn Prefix(InnerPeace __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                if(((CharacterEntity) m).stance.ID.equals("Calm")){
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPInnerPeaceCP.IsInnerPeace(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
