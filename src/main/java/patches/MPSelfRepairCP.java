...
public class MPSelfRepairCP {
    public static boolean IsSelfRepair(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof SelfRepair){
                return true;
            }
            return false;
        }
    ...
    public static class SelfRepairCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPSelfRepairCP.IsSelfRepair(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPSelfRepairCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = SelfRepair.class,
            method = "use"
    )
    public static class SelfRepairUseCP {
        public static SpireReturn Prefix(SelfRepair __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new RepairPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPSelfRepairCP.IsSelfRepair(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
