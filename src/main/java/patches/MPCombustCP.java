...
public class MPCombustCP {
    public static boolean IsCombust(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Combust){
                return true;
            }
            return false;
        }
    ...
    public static class CombustCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPCombustCP.IsCombust(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPCombustCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class CombustUseCP {
        public static SpireReturn Prefix(Combust __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new CombustPower(m, 1, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPCombustCP.IsCombust(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
