...
public class MPInfernalBladeCP {
    public static boolean IsInfernalBlade(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof InfernalBlade){
                return true;
            }
            return false;
        }
    ...
    public static class InfernalBladeCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPInfernalBladeCP.IsInfernalBlade(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPInfernalBladeCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class InfernalBladeUseCP {
        public static SpireReturn Prefix(InfernalBlade __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPInfernalBladeCP.IsInfernalBlade(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
