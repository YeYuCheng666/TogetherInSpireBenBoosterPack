...
public class MPTrueGritCP {
    public static boolean IsTrueGrit(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof TrueGrit){
                return true;
            }
            return false;
        }
    ...
    public static class TrueGritCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPTrueGritCP.IsTrueGrit(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPTrueGritCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class TrueGritUseCP {
        public static SpireReturn Prefix(TrueGrit __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ...
                if (__instance.upgraded){
                    ...
                    ((CharacterEntity) m).exhaust(1, true, false, false);
                }
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPTrueGritCP.IsTrueGrit(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
