...
public class MPMasterRealityCP {
    public static boolean IsMasterReality(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof MasterReality){
                return true;
            }
            return false;
        }
    ...
    public static class MasterRealityCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPMasterRealityCP.IsMasterReality(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPMasterRealityCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = MasterReality.class,
            method = "use"
    )
    public static class MasterRealityUseCP {
        public static SpireReturn Prefix(MasterReality __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new MasterRealityPower(m));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPMasterRealityCP.IsMasterReality(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
