...
public class MPAmplifyCP {
    public static boolean IsAmplify(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Amplify){
                return true;
            }
            return false;
        }
    ...
    public static class AmplifyCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPAmplifyCP.IsAmplify(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPAmplifyCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class AmplifyUseCP {
        public static SpireReturn Prefix(Amplify __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new AmplifyPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPAmplifyCP.IsAmplify(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
