...
public class MPDefend_BlueCP {
    public static boolean IsDefend_Blue(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Defend_Blue){
                return true;
            }
            return false;
        }
    ...
    public static class Defend_BlueCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDefend_BlueCP.IsDefend_Blue(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDefend_BlueCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class Defend_BlueUseCP {
        public static SpireReturn Prefix(Defend_Blue __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDefend_BlueCP.IsDefend_Blue(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
