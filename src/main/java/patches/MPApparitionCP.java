...
public class MPApparitionCP {
    public static boolean IsApparition(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Apparition){
                return true;
            }
            return false;
        }
    ...
    public static class ApparitionCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPApparitionCP.IsApparition(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPApparitionCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = Apparition.class,
            method = "use"
    )
    public static class ApparitionUseCP {
        public static SpireReturn Prefix(Apparition __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPApparitionCP.IsApparition(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
