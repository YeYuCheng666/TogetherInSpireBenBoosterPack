...
public class MPStaticDischargeCP {
    public static boolean IsStaticDischarge(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof StaticDischarge){
                return true;
            }
            return false;
        }
    ...
    public static class StaticDischargeCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPStaticDischargeCP.IsStaticDischarge(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPStaticDischargeCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = StaticDischarge.class,
            method = "use"
    )
    public static class StaticDischargeUseCP {
        public static SpireReturn Prefix(StaticDischarge __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new StaticDischargePower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPStaticDischargeCP.IsStaticDischarge(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
