...
public class MPPhantasmalKillerCP {
    public static boolean IsPhantasmalKiller(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof PhantasmalKiller){
                return true;
            }
            return false;
        }
    ...
    public static class PhantasmalKillerCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPPhantasmalKillerCP.IsPhantasmalKiller(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPPhantasmalKillerCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class PhantasmalKillerUseCP {
        public static SpireReturn Prefix(PhantasmalKiller __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new PhantasmalPower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPPhantasmalKillerCP.IsPhantasmalKiller(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
