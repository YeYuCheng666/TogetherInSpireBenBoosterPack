...
public class MPEnvenomCP {
    public static boolean IsEnvenom(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Envenom){
                return true;
            }
            return false;
        }
    ...
    public static class EnvenomCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPEnvenomCP.IsEnvenom(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPEnvenomCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class EnvenomUseCP {
        public static SpireReturn Prefix(Envenom __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new EnvenomPower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPEnvenomCP.IsEnvenom(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
