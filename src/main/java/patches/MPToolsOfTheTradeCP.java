...
public class MPToolsOfTheTradeCP {
    public static boolean IsToolsOfTheTrade(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof ToolsOfTheTrade){
                return true;
            }
            return false;
        }
    ...
    public static class ToolsOfTheTradeCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPToolsOfTheTradeCP.IsToolsOfTheTrade(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPToolsOfTheTradeCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ToolsOfTheTradeUseCP {
        public static SpireReturn Prefix(ToolsOfTheTrade __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new ToolsOfTheTradePower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPToolsOfTheTradeCP.IsToolsOfTheTrade(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
