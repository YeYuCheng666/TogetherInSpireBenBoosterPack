...
public class MPShrugItOffCP {
    public static boolean IsShrugItOff(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof ShrugItOff){
                return true;
            }
            return false;
        }
    ...
    public static class ShrugItOffCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPShrugItOffCP.IsShrugItOff(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPShrugItOffCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = ShrugItOff.class,
            method = "use"
    )
    public static class Defend_RedUseCP {
        public static SpireReturn Prefix(ShrugItOff __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addBlock(__instance.block);
                ((CharacterEntity) m).draw(1);
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPShrugItOffCP.IsShrugItOff(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
