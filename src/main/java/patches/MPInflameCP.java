...
public class MPInflameCP {
    public static boolean IsInflame(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Inflame){
                return true;
            }
            return false;
        }
    ...
    public static class InflameCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPInflameCP.IsInflame(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPInflameCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class InflameUseCP {
        public static SpireReturn Prefix(Inflame __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPInflameCP.IsInflame(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
