...
public class MPDarkEmbraceCP {
    public static boolean IsDarkEmbrace(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof DarkEmbrace){
                return true;
            }
            return false;
        }
    ...
    public static class DarkEmbraceCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDarkEmbraceCP.IsDarkEmbrace(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDarkEmbraceCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class DarkEmbraceUseCP {
        public static SpireReturn Prefix(DarkEmbrace __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new DarkEmbracePower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDarkEmbraceCP.IsDarkEmbrace(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
