...
public class MPCaltropsCP {
    public static boolean IsCaltrops(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Caltrops){
                return true;
            }
            return false;
        }
    ...
    public static class CaltropsCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPCaltropsCP.IsCaltrops(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPCaltropsCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class CaltropsUseCP {
        public static SpireReturn Prefix(Caltrops __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new ThornsPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPCaltropsCP.IsCaltrops(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
