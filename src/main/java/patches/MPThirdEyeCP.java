...
public class MPThirdEyeCP {
    public static boolean IsThirdEye(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof ThirdEye){
                return true;
            }
            return false;
        }
    ...
    public static class ThirdEyeCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPThirdEyeCP.IsThirdEye(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPThirdEyeCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ThirdEyeUseCP {
        public static SpireReturn Prefix(ThirdEye __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addBlock(__instance.block);
                ((CharacterEntity) m).scry(__instance.magicNumber);
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPThirdEyeCP.IsThirdEye(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
