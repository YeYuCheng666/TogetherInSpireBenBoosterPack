...
public class MPBandageUpCP {
    public static boolean IsBandageUp(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof BandageUp){
                return true;
            }
            return false;
        }
    ...
    public static class BandageUpCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBandageUpCP.IsBandageUp(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBandageUpCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class BandageUpUseCP {
        public static SpireReturn Prefix(BandageUp __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.heal(__instance.magicNumber);
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBandageUpCP.IsBandageUp(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
