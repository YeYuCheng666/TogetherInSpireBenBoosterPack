...
public class MPNirvanaCP {
    public static boolean IsNirvana(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Nirvana){
                return true;
            }
            return false;
        }
    ...
    public static class NirvanaCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPNirvanaCP.IsNirvana(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPNirvanaCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class NirvanaUseCP {
        public static SpireReturn Prefix(Nirvana __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new NirvanaPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPNirvanaCP.IsNirvana(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
