...
public class MPVigilanceCP {
    public static boolean IsVigilance(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Vigilance){
                return true;
            }
            return false;
        }
    ...
    public static class VigilanceCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPVigilanceCP.IsVigilance(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPVigilanceCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class VigilanceUseCP {
        public static SpireReturn Prefix(Vigilance __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPVigilanceCP.IsVigilance(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
