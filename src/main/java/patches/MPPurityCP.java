...
public class MPPurityCP {
    public static boolean IsPurity(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Purity){
                return true;
            }
            return false;
        }
    ...
    public static class PurityCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPPurityCP.IsPurity(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPPurityCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class PurityUseCP {
        public static SpireReturn Prefix(Purity __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).exhaust(__instance.magicNumber, false, true, true);
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPPurityCP.IsPurity(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
