...
public class MPFeelNoPainCP {
    public static boolean IsFeelNoPain(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof FeelNoPain){
                return true;
            }
            return false;
        }
    ...
    public static class FeelNoPainCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPFeelNoPainCP.IsFeelNoPain(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPFeelNoPainCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = FeelNoPain.class,
            method = "use"
    )
    public static class FeelNoPainUseCP {
        public static SpireReturn Prefix(FeelNoPain __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new FeelNoPainPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPFeelNoPainCP.IsFeelNoPain(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
