...
public class MPLeapCP {
    public static boolean IsLeap(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Leap){
                return true;
            }
            return false;
        }
    ...
    public static class LeapCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPLeapCP.IsLeap(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPLeapCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class LeapUseCP {
        public static SpireReturn Prefix(Leap __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPLeapCP.IsLeap(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
