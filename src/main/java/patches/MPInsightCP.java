...
public class MPInsightCP {
    public static boolean IsInsight(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class InsightCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPInsightCP.IsInsight(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPInsightCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class InsightUseCP {
        public static SpireReturn Prefix(Insight __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPInsightCP.IsInsight(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
