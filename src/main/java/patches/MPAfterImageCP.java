...
public class MPAfterImageCP {
    public static boolean IsAfterImage(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof AfterImage){
                return true;
            }
            return false;
        }
    ...
    public static class AfterImageCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPAfterImageCP.IsAfterImage(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPAfterImageCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class AfterImageUseCP {
        public static SpireReturn Prefix(AfterImage __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new AfterImagePower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPAfterImageCP.IsAfterImage(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
