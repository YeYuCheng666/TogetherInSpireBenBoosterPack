...
public class MPDefragmentCP {
    public static boolean IsDefragment(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Defragment){
                return true;
            }
            return false;
        }
    ...
    public static class DefragmentCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDefragmentCP.IsDefragment(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDefragmentCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = Defragment.class,
            method = "use"
    )
    public static class DefragmentUseCP {
        public static SpireReturn Prefix(Defragment __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDefragmentCP.IsDefragment(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
