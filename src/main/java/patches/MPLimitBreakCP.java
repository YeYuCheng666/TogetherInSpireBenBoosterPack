...
public class MPLimitBreakCP {
    public static boolean IsLimitBreak(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof LimitBreak){
                return true;
            }
            return false;
        }
    ...
    public static class LimitBreakCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPLimitBreakCP.IsLimitBreak(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPLimitBreakCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class LimitBreakUseCP {
        public static SpireReturn Prefix(LimitBreak __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                if (m.hasPower("Strength")){
                    m.addPower(new StrengthPower(m, m.getPower("Strength").amount));
                }
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPLimitBreakCP.IsLimitBreak(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
