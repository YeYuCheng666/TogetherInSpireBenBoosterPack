...
public class MPDefend_WatcherCP {
    public static boolean IsDefend_Watcher(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Defend_Watcher){
                return true;
            }
            return false;
        }
    ...
    public static class Defend_WatcherCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDefend_WatcherCP.IsDefend_Watcher(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDefend_WatcherCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = Defend_Watcher.class,
            method = "use"
    )
    public static class Defend_WatcherUseCP {
        public static SpireReturn Prefix(Defend_Watcher __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDefend_WatcherCP.IsDefend_Watcher(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
