...
public class MPHaltCP {
    public static boolean IsHalt(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Halt){
                return true;
            }
            return false;
        }
    ...
    public static class HaltCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPHaltCP.IsHalt(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPHaltCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class HaltUseCP {
        public static SpireReturn Prefix(Halt __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                if(((CharacterEntity) m).stance.ID.equals("Wrath")){
                    m.addBlock(__instance.block + __instance.magicNumber);
                }else{
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPHaltCP.IsHalt(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
