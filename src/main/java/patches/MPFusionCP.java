...
public class MPFusionCP {
    public static boolean IsFusion(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Fusion){
                return true;
            }
            return false;
        }
    ...
    public static class FusionCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPFusionCP.IsFusion(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPFusionCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class FusionUseCP {
        public static SpireReturn Prefix(Fusion __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                for(int i = 0; i < __instance.magicNumber; ++i) {
                    ((CharacterEntity) m).channelOrb(new Plasma(), false);
                }
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPFusionCP.IsFusion(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
