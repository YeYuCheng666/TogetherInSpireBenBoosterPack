...
public class MPWorshipCP {
    public static boolean IsWorship(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Worship){
                return true;
            }
            return false;
        }
    ...
    public static class WorshipCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPWorshipCP.IsWorship(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPWorshipCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class WorshipUseCP {
        public static SpireReturn Prefix(Worship __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPWorshipCP.IsWorship(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
