...
public class MPConsumeCP {
    public static boolean IsConsume(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class ConsumeCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPConsumeCP.IsConsume(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPConsumeCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ConsumeUseCP {
        public static SpireReturn Prefix(Consume __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new FocusPower(m, __instance.magicNumber));
                ((CharacterEntity) m).loseOrbSlot(1);
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPConsumeCP.IsConsume(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
