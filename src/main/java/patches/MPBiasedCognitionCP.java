...
public class MPBiasedCognitionCP {
    public static boolean IsBiasedCognition(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof BiasedCognition){
                return true;
            }
            return false;
        }
    ...
    public static class BiasedCognitionCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBiasedCognitionCP.IsBiasedCognition(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBiasedCognitionCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class BiasedCognitionUseCP {
        public static SpireReturn Prefix(BiasedCognition __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new FocusPower(m, __instance.magicNumber));
                m.addPower(new BiasPower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBiasedCognitionCP.IsBiasedCognition(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
