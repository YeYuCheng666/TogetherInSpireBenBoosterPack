...
public class MPBurstCP {
    public static boolean IsBurst(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Burst){
                return true;
            }
            return false;
        }
    ...
    public static class BurstCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBurstCP.IsBurst(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBurstCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class BurstUseCP {
        public static SpireReturn Prefix(Burst __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new BurstPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBurstCP.IsBurst(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
