...
public class MPPanaceaCP {
    public static boolean IsPanacea(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Panacea){
                return true;
            }
            return false;
        }
    ...
    public static class PanaceaCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPPanaceaCP.IsPanacea(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPPanaceaCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class PanaceaUseCP {
        public static SpireReturn Prefix(Panacea __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new ArtifactPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPPanaceaCP.IsPanacea(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
