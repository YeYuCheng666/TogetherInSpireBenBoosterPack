...
public class MPBetaCP {
    public static boolean IsBeta(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class BetaCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBetaCP.IsBeta(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBetaCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class BetaUseCP {
        public static SpireReturn Prefix(Beta __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                for(int i = 0; i < 1; ++i) {
                    ((CharacterEntity) m).addCard(NetworkCard.Generate(new Omega()), CardGroup.CardGroupType.DRAW_PILE);
                };
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBetaCP.IsBeta(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
