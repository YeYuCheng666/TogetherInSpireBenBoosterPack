...
public class MPFireBreathingCP {
    public static boolean IsFireBreathing(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof FireBreathing){
                return true;
            }
            return false;
        }
    ...
    public static class FireBreathingCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPFireBreathingCP.IsFireBreathing(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPFireBreathingCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = FireBreathing.class,
            method = "use"
    )
    public static class FireBreathingUseCP {
        public static SpireReturn Prefix(FireBreathing __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new FireBreathingPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPFireBreathingCP.IsFireBreathing(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
