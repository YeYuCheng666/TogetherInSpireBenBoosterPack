...
public class MPDistractionCP {
    public static boolean IsDistraction(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Distraction){
                return true;
            }
            return false;
        }
    ...
    public static class DistractionCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDistractionCP.IsDistraction(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDistractionCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class DistractionUseCP {
        public static SpireReturn Prefix(Distraction __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDistractionCP.IsDistraction(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
