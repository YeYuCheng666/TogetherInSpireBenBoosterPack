...
public class MPInfiniteBladesCP {
    public static boolean IsInfiniteBlades(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof InfiniteBlades){
                return true;
            }
            return false;
        }
    ...
    public static class InfiniteBladesCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPInfiniteBladesCP.IsInfiniteBlades(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPInfiniteBladesCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class InfiniteBladesUseCP {
        public static SpireReturn Prefix(InfiniteBlades __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new InfiniteBladesPower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPInfiniteBladesCP.IsInfiniteBlades(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
