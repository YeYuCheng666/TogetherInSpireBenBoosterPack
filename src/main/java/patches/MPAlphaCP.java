...
public class MPAlphaCP {
    public static boolean IsAlpha(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class AlphaCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPAlphaCP.IsAlpha(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPAlphaCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class AlphaUseCP {
        public static SpireReturn Prefix(Alpha __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                for(int i = 0; i < 1; ++i) {
                    ((CharacterEntity) m).addCard(NetworkCard.Generate(new Beta()), CardGroup.CardGroupType.DRAW_PILE);
                };
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPAlphaCP.IsAlpha(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
