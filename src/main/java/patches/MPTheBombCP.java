...
public class MPTheBombCP {
    public static boolean IsTheBomb(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof TheBomb){
                return true;
            }
            return false;
        }
    ...
    public static class TheBombCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPTheBombCP.IsTheBomb(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPTheBombCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class TheBombUseCP {
        public static SpireReturn Prefix(TheBomb __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new TheBombPower(m, 3, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPTheBombCP.IsTheBomb(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
