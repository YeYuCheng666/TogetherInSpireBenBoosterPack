...
public class MPMayhemCP {
    public static boolean IsMayhem(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Mayhem){
                return true;
            }
            return false;
        }
    ...
    public static class MayhemCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPMayhemCP.IsMayhem(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPMayhemCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class MayhemUseCP {
        public static SpireReturn Prefix(Mayhem __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new MayhemPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPMayhemCP.IsMayhem(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
