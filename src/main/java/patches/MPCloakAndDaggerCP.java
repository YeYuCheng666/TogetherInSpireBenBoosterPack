...
public class MPCloakAndDaggerCP {
    public static boolean IsCloakAndDagger(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof CloakAndDagger){
                return true;
            }
            return false;
        }
    ...
    public static class CloakAndDaggerCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPCloakAndDaggerCP.IsCloakAndDagger(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPCloakAndDaggerCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = CloakAndDagger.class,
            method = "use"
    )
    public static class CloakAndDaggerUseCP {
        public static SpireReturn Prefix(CloakAndDagger __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPCloakAndDaggerCP.IsCloakAndDagger(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
