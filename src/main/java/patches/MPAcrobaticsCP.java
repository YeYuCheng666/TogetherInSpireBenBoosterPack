...
public class MPAcrobaticsCP {
    public static boolean IsAcrobatics(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Acrobatics){
                return true;
            }
            return false;
        }
    ...
    public static class AcrobaticsCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPAcrobaticsCP.IsAcrobatics(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPAcrobaticsCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = Acrobatics.class,
            method = "use"
    )
    public static class AcrobaticsUseCP {
        public static SpireReturn Prefix(Acrobatics __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).draw(__instance.magicNumber);
                ((CharacterEntity) m).discard(1, false, false, false);
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPAcrobaticsCP.IsAcrobatics(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
