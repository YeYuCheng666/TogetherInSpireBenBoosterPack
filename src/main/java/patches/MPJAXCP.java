...
public class MPJAXCP {
    public static boolean IsJAX(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof JAX){
    ...
    public static class JAXCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPJAXCP.IsJAX(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPJAXCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPJAXCP.IsJAX(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
