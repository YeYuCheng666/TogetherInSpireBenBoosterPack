...
public class MPBlurCP {
    public static boolean IsBlur(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Blur){
                return true;
            }
            return false;
        }
    ...
    public static class BlurCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBlurCP.IsBlur(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBlurCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class BlurUseCP {
        public static SpireReturn Prefix(Blur __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addBlock(__instance.block);
                m.addPower(new BlurPower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBlurCP.IsBlur(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
