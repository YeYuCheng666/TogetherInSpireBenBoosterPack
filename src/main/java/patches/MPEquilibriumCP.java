...
public class MPEquilibriumCP {
    public static boolean IsEquilibrium(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Equilibrium){
                return true;
            }
            return false;
        }
    ...
    public static class EquilibriumCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPEquilibriumCP.IsEquilibrium(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPEquilibriumCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = Equilibrium.class,
            method = "use"
    )
    public static class EquilibriumUseCP {
        public static SpireReturn Prefix(Equilibrium __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addBlock(__instance.block);
                m.addPower(new EquilibriumPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPEquilibriumCP.IsEquilibrium(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
