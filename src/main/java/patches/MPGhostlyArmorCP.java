...
public class MPGhostlyArmorCP {
    public static boolean IsGhostlyArmor(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof GhostlyArmor){
                return true;
            }
            return false;
        }
    ...
    public static class GhostlyArmorCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPGhostlyArmorCP.IsGhostlyArmor(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPGhostlyArmorCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = GhostlyArmor.class,
            method = "use"
    )
    public static class GhostlyArmorUseCP {
        public static SpireReturn Prefix(GhostlyArmor __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPGhostlyArmorCP.IsGhostlyArmor(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
