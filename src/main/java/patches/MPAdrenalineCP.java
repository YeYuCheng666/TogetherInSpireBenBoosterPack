...
public class MPAdrenalineCP {
    public static boolean IsAdrenaline(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Adrenaline){
                return true;
            }
            return false;
        }
    ...
    public static class AdrenalineCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPAdrenalineCP.IsAdrenaline(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPAdrenalineCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class AdrenalineUseCP {
        public static SpireReturn Prefix(Adrenaline __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPAdrenalineCP.IsAdrenaline(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
