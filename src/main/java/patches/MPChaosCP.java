...
public class MPChaosCP {
    public static boolean IsChaos(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Chaos){
                return true;
            }
            return false;
        }
    ...
    public static class ChaosCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPChaosCP.IsChaos(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPChaosCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ChaosUseCP {
        public static SpireReturn Prefix(Chaos __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                if (__instance.upgraded) {
                    ((CharacterEntity) m).channelOrb(AbstractOrb.getRandomOrb(true), false);
                }
                ((CharacterEntity) m).channelOrb(AbstractOrb.getRandomOrb(true), false);
                return SpireReturn.Return();
            } else {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPChaosCP.IsChaos(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
