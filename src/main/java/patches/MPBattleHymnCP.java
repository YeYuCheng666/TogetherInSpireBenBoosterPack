...
public class MPBattleHymnCP {
    public static boolean IsBattleHymn(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof BattleHymn){
                return true;
            }
            return false;
        }
    ...
    public static class BattleHymnCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBattleHymnCP.IsBattleHymn(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBattleHymnCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class BattleHymnUseCP {
        public static SpireReturn Prefix(BattleHymn __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new BattleHymnPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBattleHymnCP.IsBattleHymn(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
