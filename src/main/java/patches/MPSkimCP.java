...
public class MPSkimCP {
    public static boolean IsSkim(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Skim){
                return true;
            }
            return false;
        }
    ...
    public static class SkimCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPSkimCP.IsSkim(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPSkimCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class SkimUseCP {
        public static SpireReturn Prefix(Skim __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPSkimCP.IsSkim(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
