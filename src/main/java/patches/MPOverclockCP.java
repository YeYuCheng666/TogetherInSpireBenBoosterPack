...
public class MPOverclockCP {
    public static boolean IsOverclock(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Overclock){
                return true;
            }
            return false;
        }
    ...
    public static class OverclockCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPOverclockCP.IsOverclock(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPOverclockCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class OverclockUseCP {
        public static SpireReturn Prefix(Overclock __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).draw(__instance.magicNumber);
                for(int i = 0; i < 1; ++i) {
                    ((CharacterEntity) m).addCard(NetworkCard.Generate(new Burn()), CardGroup.CardGroupType.DISCARD_PILE);
                }
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPOverclockCP.IsOverclock(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
