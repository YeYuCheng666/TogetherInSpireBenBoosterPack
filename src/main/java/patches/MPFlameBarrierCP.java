...
public class MPFlameBarrierCP {
    public static boolean IsFlameBarrier(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof FlameBarrier){
                return true;
            }
            return false;
        }
    ...
    public static class FlameBarrierCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPFlameBarrierCP.IsFlameBarrier(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPFlameBarrierCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = FlameBarrier.class,
            method = "use"
    )
    public static class FlameBarrierUseCP {
        public static SpireReturn Prefix(FlameBarrier __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addBlock(__instance.block);
                m.addPower(new FlameBarrierPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPFlameBarrierCP.IsFlameBarrier(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
