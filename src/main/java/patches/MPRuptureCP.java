...
public class MPRuptureCP {
    public static boolean IsRupture(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Rupture){
                return true;
            }
            return false;
        }
    ...
    public static class RuptureCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPRuptureCP.IsRupture(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPRuptureCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class RuptureUseCP {
        public static SpireReturn Prefix(Rupture __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new RupturePower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPRuptureCP.IsRupture(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
