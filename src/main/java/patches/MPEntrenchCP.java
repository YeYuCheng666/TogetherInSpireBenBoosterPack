...
public class MPEntrenchCP {
    public static boolean IsEntrench(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Entrench){
                return true;
            }
            return false;
        }
    ...
    public static class EntrenchCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPEntrenchCP.IsEntrench(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPEntrenchCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class EntrenchUseCP {
        public static SpireReturn Prefix(Entrench __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addBlock(m.currentBlock);
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPEntrenchCP.IsEntrench(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
