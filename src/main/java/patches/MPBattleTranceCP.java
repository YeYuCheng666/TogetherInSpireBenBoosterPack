...
public class MPBattleTranceCP {
    public static boolean IsBattleTrance(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof BattleTrance){
                return true;
            }
            return false;
        }
    ...
    public static class BattleTranceCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBattleTranceCP.IsBattleTrance(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBattleTranceCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = BattleTrance.class,
            method = "use"
    )
    public static class BattleTranceUseCP {
        public static SpireReturn Prefix(BattleTrance __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).draw(__instance.magicNumber);
                m.addPower(new NoDrawPower(m));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBattleTranceCP.IsBattleTrance(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
