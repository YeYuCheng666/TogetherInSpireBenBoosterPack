...
public class MPAutoShieldsCP {
    public static boolean IsAutoShields(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof AutoShields){
                return true;
            }
            return false;
        }
    ...
    public static class AutoShieldsCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPAutoShieldsCP.IsAutoShields(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPAutoShieldsCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class AutoShieldsUseCP {
        public static SpireReturn Prefix(AutoShields __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPAutoShieldsCP.IsAutoShields(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
