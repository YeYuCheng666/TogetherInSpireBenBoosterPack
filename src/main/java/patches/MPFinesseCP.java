...
public class MPFinesseCP {
    public static boolean IsFinesse(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Finesse){
                return true;
            }
            return false;
        }
    ...
    public static class FinesseCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPFinesseCP.IsFinesse(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPFinesseCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class FinesseUseCP {
        public static SpireReturn Prefix(Finesse __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPFinesseCP.IsFinesse(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
