...
public class MPWellLaidPlansCP {
    public static boolean IsWellLaidPlans(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof WellLaidPlans){
                return true;
            }
            return false;
        }
    ...
    public static class WellLaidPlansCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPWellLaidPlansCP.IsWellLaidPlans(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPWellLaidPlansCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = WellLaidPlans.class,
            method = "use"
    )
    public static class WellLaidPlansUseCP {
        public static SpireReturn Prefix(WellLaidPlans __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new RetainCardPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPWellLaidPlansCP.IsWellLaidPlans(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
