...
public class MPCreativeAICP {
    public static boolean IsCreativeAI(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof CreativeAI){
                return true;
            }
            return false;
        }
    ...
    public static class CreativeAICP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPCreativeAICP.IsCreativeAI(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPCreativeAICP.queueContains(__instance.hoveredCard)) {
    ...
    public static class CreativeAIUseCP {
        public static SpireReturn Prefix(CreativeAI __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new CreativeAIPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPCreativeAICP.IsCreativeAI(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
