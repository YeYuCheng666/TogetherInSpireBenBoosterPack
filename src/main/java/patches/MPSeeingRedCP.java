...
public class MPSeeingRedCP {
    public static boolean IsSeeingRed(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof SeeingRed){
                return true;
            }
            return false;
        }
    ...
    public static class SeeingRedCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPSeeingRedCP.IsSeeingRed(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPSeeingRedCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class SeeingRedUseCP {
        public static SpireReturn Prefix(SeeingRed __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPSeeingRedCP.IsSeeingRed(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
