...
public class MPLikeWaterCP {
    public static boolean IsLikeWater(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof LikeWater){
                return true;
            }
            return false;
        }
    ...
    public static class LikeWaterCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPLikeWaterCP.IsLikeWater(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPLikeWaterCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class LikeWaterUseCP {
        public static SpireReturn Prefix(LikeWater __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new LikeWaterPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPLikeWaterCP.IsLikeWater(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
