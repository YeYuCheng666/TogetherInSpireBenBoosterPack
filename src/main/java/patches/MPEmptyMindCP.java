...
public class MPEmptyMindCP {
    public static boolean IsEmptyMind(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof EmptyMind){
                return true;
            }
            return false;
        }
    ...
    public static class EmptyMindCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPEmptyMindCP.IsEmptyMind(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPEmptyMindCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class EmptyMindUseCP {
        public static SpireReturn Prefix(EmptyMind __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).draw(__instance.magicNumber);
                ((CharacterEntity) m).switchStance(new NeutralStance());
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPEmptyMindCP.IsEmptyMind(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
