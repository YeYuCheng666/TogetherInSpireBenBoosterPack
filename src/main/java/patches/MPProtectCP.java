...
public class MPProtectCP {
    public static boolean IsProtect(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class ProtectCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPProtectCP.IsProtect(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPProtectCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ProtectUseCP {
        public static SpireReturn Prefix(Protect __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPProtectCP.IsProtect(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
