...
public class MPRushdownCP {
    public static boolean IsRushdown(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Rushdown){
                return true;
            }
            return false;
        }
    ...
    public static class RushdownCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPRushdownCP.IsRushdown(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPRushdownCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class RushdownUseCP {
        public static SpireReturn Prefix(Rushdown __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new RushdownPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPRushdownCP.IsRushdown(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
