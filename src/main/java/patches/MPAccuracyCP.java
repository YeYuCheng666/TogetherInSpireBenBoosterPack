...
public class MPAccuracyCP {
    public static boolean IsAccuracy(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class AccuracyCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPAccuracyCP.IsAccuracy(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPAccuracyCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class AccuracyUseCP {
        public static SpireReturn Prefix(Accuracy __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new AccuracyPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPAccuracyCP.IsAccuracy(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
