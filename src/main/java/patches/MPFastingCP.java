...
public class MPFastingCP {
    public static boolean IsFasting(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Fasting){
                return true;
            }
            return false;
        }
    ...
    public static class FastingCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPFastingCP.IsFasting(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPFastingCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class FastingUseCP {
        public static SpireReturn Prefix(Fasting __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ...
                m.addPower(new EnergyDownPower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPFastingCP.IsFasting(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
