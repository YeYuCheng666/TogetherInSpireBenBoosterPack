...
public class MPDefend_GreenCP {
    public static boolean IsDefend_Green(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Defend_Green){
                return true;
            }
            return false;
        }
    ...
    public static class Defend_GreenCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDefend_GreenCP.IsDefend_Green(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDefend_GreenCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class Defend_GreenUseCP {
        public static SpireReturn Prefix(Defend_Green __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDefend_GreenCP.IsDefend_Green(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
