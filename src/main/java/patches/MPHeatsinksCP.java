...
public class MPHeatsinksCP {
    public static boolean IsHeatsinks(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Heatsinks){
                return true;
            }
            return false;
        }
    ...
    public static class HeatsinksCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPHeatsinksCP.IsHeatsinks(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPHeatsinksCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class HeatsinksUseCP {
        public static SpireReturn Prefix(Heatsinks __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new HeatsinkPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPHeatsinksCP.IsHeatsinks(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
