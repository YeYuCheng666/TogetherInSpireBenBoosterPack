...
public class MPJuggernautCP {
    public static boolean IsJuggernaut(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Juggernaut){
                return true;
            }
            return false;
        }
    ...
    public static class JuggernautCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPJuggernautCP.IsJuggernaut(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPJuggernautCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = Juggernaut.class,
            method = "use"
    )
    public static class JuggernautUseCP {
        public static SpireReturn Prefix(Juggernaut __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new JuggernautPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPJuggernautCP.IsJuggernaut(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
