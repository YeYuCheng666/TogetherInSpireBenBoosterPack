...
public class MPEvolveCP {
    public static boolean IsEvolve(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Evolve){
                return true;
            }
            return false;
        }
    ...
    public static class EvolveCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPEvolveCP.IsEvolve(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPEvolveCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class EvolveUseCP {
        public static SpireReturn Prefix(Evolve __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new EvolvePower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPEvolveCP.IsEvolve(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
