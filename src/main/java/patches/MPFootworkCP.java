...
public class MPFootworkCP {
    public static boolean IsFootwork(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Footwork){
                return true;
            }
            return false;
        }
    ...
    public static class FootworkCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPFootworkCP.IsFootwork(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPFootworkCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class FootworkUseCP {
        public static SpireReturn Prefix(Footwork __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPFootworkCP.IsFootwork(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
