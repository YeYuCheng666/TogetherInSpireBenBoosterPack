...
public class MPSimmeringFuryCP {
    public static boolean IsSimmeringFury(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof SimmeringFury){
                return true;
            }
            return false;
        }
    ...
    public static class SimmeringFuryCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPSimmeringFuryCP.IsSimmeringFury(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPSimmeringFuryCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = SimmeringFury.class,
            method = "use"
    )
    public static class SimmeringFuryUseCP {
        public static SpireReturn Prefix(SimmeringFury __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new WrathNextTurnPower(m));
                m.addPower(new DrawCardNextTurnPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPSimmeringFuryCP.IsSimmeringFury(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
