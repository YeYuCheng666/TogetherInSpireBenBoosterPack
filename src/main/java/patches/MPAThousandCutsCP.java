...
public class MPAThousandCutsCP {
    public static boolean IsAThousandCuts(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof AThousandCuts){
                return true;
            }
            return false;
        }
    ...
    public static class AThousandCutsCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPAThousandCutsCP.IsAThousandCuts(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPAThousandCutsCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class AThousandCutsUseCP {
        public static SpireReturn Prefix(AThousandCuts __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new ThousandCutsPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPAThousandCutsCP.IsAThousandCuts(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
