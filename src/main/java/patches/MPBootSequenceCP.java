...
public class MPBootSequenceCP {
    public static boolean IsBootSequence(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof BootSequence){
                return true;
            }
            return false;
        }
    ...
    public static class BootSequenceCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBootSequenceCP.IsBootSequence(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBootSequenceCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = BootSequence.class,
            method = "use"
    )
    public static class BootSequenceUseCP {
        public static SpireReturn Prefix(BootSequence __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBootSequenceCP.IsBootSequence(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
