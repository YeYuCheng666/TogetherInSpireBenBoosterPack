...
public class MPForesightCP {
    public static boolean IsForesight(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Foresight){
                return true;
            }
            return false;
        }
    ...
    public static class ForesightCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPForesightCP.IsForesight(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPForesightCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ForesightUseCP {
        public static SpireReturn Prefix(Foresight __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new ForesightPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPForesightCP.IsForesight(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
