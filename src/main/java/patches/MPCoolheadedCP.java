...
public class MPCoolheadedCP {
    public static boolean IsCoolheaded(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Coolheaded){
                return true;
            }
            return false;
        }
    ...
    public static class CoolheadedCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPCoolheadedCP.IsCoolheaded(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPCoolheadedCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = Coolheaded.class,
            method = "use"
    )
    public static class CoolheadedUseCP {
        public static SpireReturn Prefix(Coolheaded __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPCoolheadedCP.IsCoolheaded(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
