...
public class MPEchoFormCP {
    public static boolean IsEchoForm(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof EchoForm){
                return true;
            }
            return false;
        }
    ...
    public static class EchoFormCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPEchoFormCP.IsEchoForm(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPEchoFormCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class EchoFormUseCP {
        public static SpireReturn Prefix(EchoForm __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new EchoPower(m, 1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPEchoFormCP.IsEchoForm(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
