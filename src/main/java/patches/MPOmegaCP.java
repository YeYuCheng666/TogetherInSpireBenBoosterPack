...
public class MPOmegaCP {
    public static boolean IsOmega(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Omega){
                return true;
            }
            return false;
        }
    ...
    public static class OmegaCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPOmegaCP.IsOmega(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPOmegaCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class OmegaUseCP {
        public static SpireReturn Prefix(Omega __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new OmegaPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPOmegaCP.IsOmega(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
