...
public class MPMasterOfStrategyCP {
    public static boolean IsMasterOfStrategy(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof MasterOfStrategy){
                return true;
            }
            return false;
        }
    ...
    public static class MasterOfStrategyCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPMasterOfStrategyCP.IsMasterOfStrategy(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPMasterOfStrategyCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = MasterOfStrategy.class,
            method = "use"
    )
    public static class MasterOfStrategyUseCP {
        public static SpireReturn Prefix(MasterOfStrategy __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPMasterOfStrategyCP.IsMasterOfStrategy(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
