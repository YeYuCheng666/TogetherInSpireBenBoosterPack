...
public class MPSafetyCP {
    public static boolean IsSafety(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class SafetyCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPSafetyCP.IsSafety(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPSafetyCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class SafetyUseCP {
        public static SpireReturn Prefix(Safety __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPSafetyCP.IsSafety(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
