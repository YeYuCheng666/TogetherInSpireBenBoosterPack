...
public class MPSadisticNatureCP {
    public static boolean IsSadisticNature(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof SadisticNature){
                return true;
            }
            return false;
        }
    ...
    public static class SadisticNatureCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPSadisticNatureCP.IsSadisticNature(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPSadisticNatureCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = SadisticNature.class,
            method = "use"
    )
    public static class SadisticNatureUseCP {
        public static SpireReturn Prefix(SadisticNature __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new SadisticPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPSadisticNatureCP.IsSadisticNature(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
