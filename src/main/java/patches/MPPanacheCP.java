...
public class MPPanacheCP {
    public static boolean IsPanache(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Panache){
                return true;
            }
            return false;
        }
    ...
    public static class PanacheCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPPanacheCP.IsPanache(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPPanacheCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class PanacheUseCP {
        public static SpireReturn Prefix(Panache __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new PanachePower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPPanacheCP.IsPanache(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
