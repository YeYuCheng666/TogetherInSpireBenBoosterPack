...
public class MPFlexCP {
    public static boolean IsFlex(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Flex){
                return true;
            }
            return false;
        }
    ...
    public static class FlexCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPFlexCP.IsFlex(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPFlexCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class FlexUseCP {
        public static SpireReturn Prefix(Flex __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new StrengthPower(m, __instance.magicNumber));
                m.addPower(new LoseStrengthPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPFlexCP.IsFlex(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
