...
public class MPBlasphemyCP {
    public static boolean IsBlasphemy(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Blasphemy){
                return true;
            }
            return false;
        }
    ...
    public static class BlasphemyCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBlasphemyCP.IsBlasphemy(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBlasphemyCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class BlasphemyUseCP {
        public static SpireReturn Prefix(Blasphemy __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).switchStance(new DivinityStance());
                m.addPower(new EndTurnDeathPower(m));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBlasphemyCP.IsBlasphemy(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
