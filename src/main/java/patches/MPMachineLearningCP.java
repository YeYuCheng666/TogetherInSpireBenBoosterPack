...
public class MPMachineLearningCP {
    public static boolean IsMachineLearning(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof MachineLearning){
                return true;
            }
            return false;
        }
    ...
    public static class MachineLearningCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPMachineLearningCP.IsMachineLearning(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPMachineLearningCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class MachineLearningUseCP {
        public static SpireReturn Prefix(MachineLearning __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new DrawPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPMachineLearningCP.IsMachineLearning(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
