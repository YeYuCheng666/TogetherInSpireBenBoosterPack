...
public class MPConcentrateCP {
    public static boolean IsConcentrate(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Concentrate){
                return true;
            }
            return false;
        }
    ...
    public static class ConcentrateCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPConcentrateCP.IsConcentrate(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPConcentrateCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ConcentrateUseCP {
        public static SpireReturn Prefix(Concentrate __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).discard(__instance.magicNumber, false, false, false);
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPConcentrateCP.IsConcentrate(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
