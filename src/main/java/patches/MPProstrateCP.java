...
public class MPProstrateCP {
    public static boolean IsProstrate(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Prostrate){
                return true;
            }
            return false;
        }
    ...
    public static class ProstrateCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPProstrateCP.IsProstrate(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPProstrateCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ProstrateUseCP {
        public static SpireReturn Prefix(Prostrate __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPProstrateCP.IsProstrate(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
