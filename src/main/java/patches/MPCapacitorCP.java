...
public class MPCapacitorCP {
    public static boolean IsCapacitor(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class CapacitorCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPCapacitorCP.IsCapacitor(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPCapacitorCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class CapacitorUseCP {
        public static SpireReturn Prefix(Capacitor __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).gainOrbSlot(__instance.magicNumber);
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPCapacitorCP.IsCapacitor(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
