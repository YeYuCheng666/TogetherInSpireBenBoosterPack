...
public class MPBloodlettingCP {
    public static boolean IsBloodletting(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Bloodletting){
                return true;
            }
            return false;
        }
    ...
    public static class BloodlettingCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPBloodlettingCP.IsBloodletting(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPBloodlettingCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPBloodlettingCP.IsBloodletting(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
