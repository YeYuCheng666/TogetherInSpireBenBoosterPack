...
public class MPGoodInstinctsCP {
    public static boolean IsGoodInstincts(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof GoodInstincts){
                return true;
            }
            return false;
        }
    ...
    public static class GoodInstinctsCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPGoodInstinctsCP.IsGoodInstincts(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPGoodInstinctsCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = GoodInstincts.class,
            method = "use"
    )
    public static class GoodInstinctsUseCP {
        public static SpireReturn Prefix(GoodInstincts __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPGoodInstinctsCP.IsGoodInstincts(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
