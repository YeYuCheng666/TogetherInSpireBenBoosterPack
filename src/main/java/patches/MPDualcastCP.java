...
public class MPDualcastCP {
    public static boolean IsDualcast(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Dualcast){
                return true;
            }
            return false;
        }
    ...
    public static class DualcastCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDualcastCP.IsDualcast(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDualcastCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class DualcastUseCP {
        public static SpireReturn Prefix(Dualcast __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ((CharacterEntity) m).evokeOrb(1, 1, null, false, false);
                ((CharacterEntity) m).evokeOrb(1, 1, null, false, true);
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDualcastCP.IsDualcast(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
