...
public class MPDodgeAndRollCP {
    public static boolean IsDodgeAndRoll(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof DodgeAndRoll){
                return true;
            }
            return false;
        }
    ...
    public static class DodgeAndRollCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPDodgeAndRollCP.IsDodgeAndRoll(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPDodgeAndRollCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = DodgeAndRoll.class,
            method = "use"
    )
    public static class DodgeAndRollUseCP {
        public static SpireReturn Prefix(DodgeAndRoll __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addBlock(__instance.block);
                m.addPower(new NextTurnBlockPower(m, __instance.block));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPDodgeAndRollCP.IsDodgeAndRoll(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
