...
public class MPZapCP {
    public static boolean IsZap(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Zap){
    ...
    public static class ZapCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPZapCP.IsZap(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPZapCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class ZapUseCP {
        public static SpireReturn Prefix(Zap __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPZapCP.IsZap(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
