...
public class MPNoxiousFumesCP {
    public static boolean IsNoxiousFumes(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof NoxiousFumes){
                return true;
            }
            return false;
        }
    ...
    public static class NoxiousFumesCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPNoxiousFumesCP.IsNoxiousFumes(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPNoxiousFumesCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = NoxiousFumes.class,
            method = "use"
    )
    public static class NoxiousFumesUseCP {
        public static SpireReturn Prefix(NoxiousFumes __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new NoxiousFumesPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPNoxiousFumesCP.IsNoxiousFumes(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
