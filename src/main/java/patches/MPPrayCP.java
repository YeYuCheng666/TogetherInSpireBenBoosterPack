...
public class MPPrayCP {
    public static boolean IsPray(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class PrayCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPPrayCP.IsPray(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPPrayCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class PrayUseCP {
        public static SpireReturn Prefix(Pray __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new MantraPower(m, __instance.magicNumber));
                for(int i = 0; i < 1; ++i) {
                    ((CharacterEntity) m).addCard(NetworkCard.Generate(new Insight()), CardGroup.CardGroupType.DRAW_PILE);
                }
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPPrayCP.IsPray(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
