...
public class MPEmptyBodyCP {
    public static boolean IsEmptyBody(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof EmptyBody){
                return true;
            }
            return false;
        }
    ...
    public static class EmptyBodyCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPEmptyBodyCP.IsEmptyBody(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPEmptyBodyCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class EmptyBodyUseCP {
        public static SpireReturn Prefix(EmptyBody __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPEmptyBodyCP.IsEmptyBody(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
