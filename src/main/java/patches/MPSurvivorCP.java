...
public class MPSurvivorCP {
    public static boolean IsSurvivor(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class SurvivorCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPSurvivorCP.IsSurvivor(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPSurvivorCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class SurvivorUseCP {
        public static SpireReturn Prefix(Survivor __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPSurvivorCP.IsSurvivor(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
