...
public class MPSentinelCP {
    public static boolean IsSentinel(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
    ...
    public static class SentinelCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPSentinelCP.IsSentinel(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPSentinelCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class SentinelUseCP {
        public static SpireReturn Prefix(Sentinel __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPSentinelCP.IsSentinel(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
