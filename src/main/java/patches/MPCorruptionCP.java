...
public class MPCorruptionCP {
    public static boolean IsCorruption(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Corruption){
                return true;
            }
            return false;
        }
    ...
    public static class CorruptionCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPCorruptionCP.IsCorruption(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPCorruptionCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = Corruption.class,
            method = "use"
    )
    public static class CorruptionUseCP {
        public static SpireReturn Prefix(Corruption __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ...
                if (!powerExists) {
                    m.addPower(new CorruptionPower(m));
                }
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPCorruptionCP.IsCorruption(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
