...
public class MPImperviousCP {
    public static boolean IsImpervious(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof Impervious){
                return true;
            }
            return false;
        }
    ...
    public static class ImperviousCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPImperviousCP.IsImpervious(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPImperviousCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = Impervious.class,
            method = "use"
    )
    public static class ImperviousUseCP {
        public static SpireReturn Prefix(Impervious __instance, AbstractPlayer p, AbstractMonster m) {
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPImperviousCP.IsImpervious(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
