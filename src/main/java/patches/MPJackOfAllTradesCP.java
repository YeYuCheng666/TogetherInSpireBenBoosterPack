...
public class MPJackOfAllTradesCP {
    public static boolean IsJackOfAllTrades(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof JackOfAllTrades){
                return true;
            }
            return false;
        }
    ...
    public static class JackOfAllTradesCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPJackOfAllTradesCP.IsJackOfAllTrades(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPJackOfAllTradesCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class JackOfAllTradesUseCP {
        public static SpireReturn Prefix(JackOfAllTrades __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                ...
                ((CharacterEntity) m).addCard(NetworkCard.Generate(c), CardGroup.CardGroupType.HAND);
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPJackOfAllTradesCP.IsJackOfAllTrades(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
