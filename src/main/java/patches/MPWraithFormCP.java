...
public class MPWraithFormCP {
    public static boolean IsWraithForm(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof WraithForm){
                return true;
            }
            return false;
        }
    ...
    public static class WraithFormCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPWraithFormCP.IsWraithForm(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPWraithFormCP.queueContains(__instance.hoveredCard)) {
    ...
    public static class WraithFormUseCP {
        public static SpireReturn Prefix(WraithForm __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new IntangiblePlayerPower(m, __instance.magicNumber));
                m.addPower(new WraithFormPower(m, -1));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPWraithFormCP.IsWraithForm(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
