...
public class MPMentalFortressCP {
    public static boolean IsMentalFortress(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof MentalFortress){
                return true;
            }
            return false;
        }
    ...
    public static class MentalFortressCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPMentalFortressCP.IsMentalFortress(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPMentalFortressCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = MentalFortress.class,
            method = "use"
    )
    public static class MentalFortressUseCP {
        public static SpireReturn Prefix(MentalFortress __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new MentalFortressPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPMentalFortressCP.IsMentalFortress(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
