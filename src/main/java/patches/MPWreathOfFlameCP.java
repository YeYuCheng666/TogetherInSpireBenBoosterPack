...
public class MPWreathOfFlameCP {
    public static boolean IsWreathOfFlame(AbstractCard o) {
        if (o == null) {
            return false;
        } else {
            if (o instanceof WreathOfFlame){
                return true;
            }
            return false;
        }
    ...
    public static class WreathOfFlameCP {
        public static SpireReturn Prefix(AbstractPlayer __instance) {
            if (SpireTogetherMod.isConnected && MPWreathOfFlameCP.IsWreathOfFlame(__instance.hoveredCard)) {
                InputHelper.justClickedLeft = false;
                __instance.hoverEnemyWaitTimer = 1.0F;
                __instance.hoveredCard.unhover();
                if (!MPWreathOfFlameCP.queueContains(__instance.hoveredCard)) {
    ...
    @SpirePatch(
            clz = WreathOfFlame.class,
            method = "use"
    )
    public static class WreathOfFlameUseCP {
        public static SpireReturn Prefix(WreathOfFlame __instance, AbstractPlayer p, AbstractMonster m) {
            if (SpireTogetherMod.isConnected && m instanceof CharacterEntity) {
                m.addPower(new VigorPower(m, __instance.magicNumber));
                return SpireReturn.Return();
            } else {
                return SpireReturn.Continue();
            }
    ...
    public static class TargetPlayersClass {
        public static void Prefix(AbstractCard __instance) {
            if (SpireTogetherMod.isConnected && MPWreathOfFlameCP.IsWreathOfFlame(__instance) && AbstractDungeon.player.isDraggingCard && __instance == AbstractDungeon.player.hoveredCard) {
